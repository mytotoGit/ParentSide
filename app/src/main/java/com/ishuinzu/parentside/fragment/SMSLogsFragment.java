...
import com.ishuinzu.parentside.databinding.FragmentSMSLogsBinding;
...
public class SMSLogsFragment extends Fragment implements View.OnClickListener {
    private static final String TAG = "SMSLogsFragment";
    ...
    private List<List<SMSObject>> listConversations;
    private List<String> listAddresses;

    public SMSLogsFragment() {
    }

    public static SMSLogsFragment newInstance(String param1, String param2) {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        binding = FragmentSMSLogsBinding.inflate(inflater);
    ...
    private void loadConversations(String id) {
        ...
        conversationAdapter = new ConversationAdapter(getContext(), conversationObjects, "SMS");
        binding.recyclerConversations.setAdapter(conversationAdapter);

        getConversationLogs(id);
    }

    private void updateProgresses(int total) {
        binding.progressTotalSMS.setProgress(total, (total * 2));
    }
                        ...
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            ...
                            if (snapshot.getChildrenCount() != 0) {
                                for (DataSnapshot dataSnapshot : snapshot.getChildren()) {
                                    ...
                                    if (smsObject != null) {
                                        listConversations.get(finalI).add(smsObject);
                                    }
                                }
